...
export const TestNestedType = TestNestedSchema.pipe(
  MosesObject("example.com/type/TestNested", "0.1.0"),
);
...
// TODO(BEEPHOLE!): Why do we use need this rather then TestSchemaType?
export const TestType = TestSchema.pipe(
  MosesObject("example.com/type/Test", "0.1.0"),
);
...
// TODO(ben): Another top-level S.mutable call as a workaround for the regression in the last minor.
...
export class Contact extends TypedObject({
...
})(
  {
    ...
    email: S.String,
    tasks: S.suspend(
      (): S.mutable<S.Array$<Ref$<Task>>> => S.mutable(S.Array(Ref(Task))),
    ),
    address: S.Struct({
...
export class Task extends TypedObject({
  ...
  // TODO(BEEPHOLE!): Document S.suspend.
  subTasks: S.optional(
    S.mutable(S.Array(S.suspend((): Ref$<Task> => Ref(Task)))),
  ),
