...
const encodeMultipleOf = (divisor: number) => 1 / 10 ** divisor;
...
const encodeMultiple =
  ...
  <I, R>(self: S.Schema<A, I, R>) =>
    divisor === undefined || divisor === 0
      ? self
      : self.pipe(S.multipleOf(encodeMultipleOf(divisor)));
...
export const CurrencyAnnotationId = Symbol.for(
  "@moses/schema/annotation/Currency",
);
...
export const Currency = (
  { decimals, code }: CurrencyAnnotation = { decimals: 2 },
) =>
 ...
 * Percent.
 */
// TODO(BEEPHOLE!): Define min/max (e.g., 0, 1).
