...
// TODO(BEEPHOLE!): Reuse/reconcile with ScalarTypeEnum (handle arrays).
const SimpleTypes = S.Literal(
  "array",
  "boolean",
  "integer",
  "null",
...
/**
 * Describes a schema for the JSON-schema objects stored in MOSES.
 * Contains extensions for MOSES (e.g., references).
...
const _JsonSchemaType = S.mutable(
  S.Struct({
     ...
     * Only on schema representing a MOSES object.
     *
     ...
     */
    pattern: S.optional(
      S.String.annotations({ [FormatAnnotationId]: "regex" }),
    ),
     ...
     * @deprecated
     */
    // TODO(ben): Extract `annotations` and remove the namespace property.
    moses: S.optional(
      S.mutable(
        S.Struct({
          /**
           * @deprecated
           */
          // TODO(ben): We risk old schema not passing validation due to the extra fields. Remove when we are sure this is safe
          type: S.optional(
            S.mutable(
              S.Struct({
                typename: S.String,
...
// TODO(BEEPHOLE!): Factor out JSON schema utils.
...
// TODO(BEEPHOLE!): Properties should be ordered.
