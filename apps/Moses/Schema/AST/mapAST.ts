...
export const getSimpleType = (node: AST.AST): SimpleType | undefined => {
  if (
    AST.isObjectKeyword(node) ||
    AST.isTypeLiteral(node) ||
...
export namespace SimpleType {
   ...
   */
  export const getDefaultValue = (type: SimpleType): MTypes.AnyType => {
    switch (type) {
      case "string": {
        return "";
      }
...
export const visit: {
  ...
  node: AST.AST,
  testOrVisitor: TestFn | VisitorFn,
  visitor?: VisitorFn,
): void => {
  if (!visitor) {
...
export const getAnnotation =
  ...
  (node: AST.AST): T | undefined => {
    ...
    const value = pipe(
      AST.getAnnotation<T>(annotationId)(node),
      Option.getOrUndefined,
    );
    if (
      noDefault &&
      (value === defaultAnnotations[node._tag]?.annotations[annotationId] ||
        value === id)
    ) {
      return undefined;
    }
...
/**
 * Recursively descend into AST to find the first matching annotations.
...
export const isOption = (node: AST.AST): boolean => {
  return (
    AST.isUnion(node) &&
    node.types.length === 2 &&
    AST.isUndefinedKeyword(node.types[1])
  );
};
...
export const getDiscriminatingProps = (node: AST.AST): string[] | undefined => {
  ...
  return node.types.reduce<string[]>((shared, type) => {
    ...
    return shared.length === 0
      ? props
      : shared.filter((prop) => props.includes(prop));
  }, []);
};
...
export const getDiscriminatedType = (
  node: AST.AST,
  value: Record<string, MTypes.AnyType> = {},
): AST.AST | undefined => {
  invariant(AST.isUnion(node));
  invariant(value);
      ...
      .map((prop) => {
        const literals = node.types
          .map((type) => {
            ...
            const literal = AST.getPropertySignatures(type).find(
              (p) => p.name.toString() === prop,
            )!;
...
export const mapAst = (
  ast: AST.AST,
  f: (ast: AST.AST, key: keyof MTypes.AnyType | undefined) => AST.AST,
): AST.AST => {
  switch (ast._tag) {
    case "TypeLiteral":
    ...
    case "TupleType":
      return new AST.TupleType(
        ast.elements.map(
          (t, index) =>
            new AST.OptionalType(f(t.type, index), t.isOptional, t.annotations),
        ),
