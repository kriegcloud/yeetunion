...
import { ValidationError, compose, define } from "../src/primitives/Builder";
...
describe("Builder v0.3.0", () => {
  ...
  const withRole = (role: string) =>
    User.roles.modify((roles) => [...roles, role]);

  describe("Field Operations", () => {
    it.effect("should set field values", () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          compose(User.name("John"), User.age(30)),
          User.build,
        );
    ...
    it.effect("should modify field values", () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          compose(withRole("user"), withRole("admin")),
          User.build,
        );
  ...
  describe("Schema Validation", () => {
    it.effect("should validate required fields", () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          compose(User.name("John"), User.age(-1)),
          User.build,
          Effect.either,
        );
  ...
  describe("Default Values", () => {
    it.effect("should use default values when fields are not set", () =>
      Effect.gen(function* () {
        const result = yield* pipe(compose(User.name("John")), User.build);
  ...
  describe("Schema Defaults", () => {
    it.effect("should respect field-level schema defaults", () =>
      Effect.gen(function* () {
        ...
        const result = yield* pipe(compose(builder.count(100)), builder.build);
    ...
    it.effect("should override schema defaults with builder defaults", () =>
      Effect.gen(function* () {
        ...
        const result = yield* pipe(compose(), builder.build);
