...
import { MiddlewareChainContext, ChainableMiddleware } from "../compose/types";
import {
  MiddlewareBuilder,
  MiddlewareConfig,
  WithoutBoolUnions,
...
export const unpackConfig = async <Config extends Record<string, unknown>>(
  ...
  const userAgent: NextUserAgent = nextUserAgent({ headers: req.headers as any });
...
const mergeConfigs =
  <Config extends Record<string, unknown>>(
    ...
    keyMerger: KeyMerger = (_k: any, _l: any, r: any) => r,
...
const defaultConfigMergers: KeyMerger[] = [
  ...
  (_: string, l: any, r: any) => (isTrue(r) && isNonBool(l) ? l : undefined),
  // nullify default config on false flags
  (_: string, l: any, r: any) => (isFalse(r) && isNonBool(l) ? null : undefined),
  // if all mergers returned undefined, choose right value
  (_: string, _l: any, r: any) => r,
];
